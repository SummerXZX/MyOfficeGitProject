...
+(void)creatCallRecordTable
{
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:path])
    {
        ...
        if ([callRecordDB open])
        {
            ...
            NSString *creatLocalDicTableSqlStr = @"CREATE TABLE CallRecord (calluserid varchar(100) PRIMARY KEY,callnum varchar(20),callname varchar(20),callavatar varchar(200),calltime int)";
...
+(BOOL)updateCallRecordWith:(LoginUserInfo *)userInfo{
    NSString *path = [CallRecordDBManager getCallRecordDBPath];
    path = [path stringByAppendingPathComponent:@"CallRecord.db"];
    FMDatabase *callRecordDB = [[FMDatabase alloc]initWithPath:path];
    if ([callRecordDB open])
    {
        ...
        NSString *updateSql = [NSString stringWithFormat:@"REPLACE INTO CallRecord VALUES ('%@','%@','%@','%@',%ld)",userInfo.userCode,userInfo.userPhone,userInfo.userName,userInfo.userIcon,(long)[[NSDate date] timeIntervalSince1970]];
//        NSString *checkSql = [NSString stringWithFormat:@"SELECT * FROM CallRecord WHERE calluserid = '%@'",userInfo.userCode];
...
+(NSArray *)getCallRecordArr
{
    ...
    if ([callRecordDB open])
    {
        ...
        while ([reslut next])
        {
            NSString *calluserid = [reslut stringForColumn:@"calluserid"];
        ...
        [tempArr addObject:@{@"calluserid":calluserid,@"callnum":callnum,@"callname":callname,@"callavatar":callavatar,@"calltime":[NSNumber numberWithInt:calltime]}];
        }
        [callRecordDB close];
    }
