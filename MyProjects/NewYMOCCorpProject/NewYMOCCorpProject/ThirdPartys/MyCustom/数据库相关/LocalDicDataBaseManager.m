...
+ (void)insertLocalDicDataWith:(NSArray*)arr
{
    ...
    if ([localDicDB open]) {
        for (NSInteger i = 0; i < arr.count; i++) {
            ...
            for (NSInteger j = 0; j < versionsArr.count; j++) {
...
+ (NSArray*)getLocalVerArr
{
    ...
    if ([localDicDB open]) {
        ...
        while ([result next]) {
            NSInteger dicVer = [result intForColumn:@"dicVer"];
            NSString* dicName = [result stringForColumn:@"dicName"];
            [tempArr addObject:@{ @"tableName" : dicName,
                @"tableVer" : [NSNumber numberWithInteger:dicVer] }];
        }
    }
    [localDicDB close];
...
+ (NSArray*)getVersionArrWithDicType:(LocalDicType)dicType
{
    ...
    if ([localDicDB open]) {
        ...
        while ([result next]) {
            NSInteger versionsId = [result intForColumn:@"versionsId"];
            NSString* nameStr = [result stringForColumn:@"name"];
            [tempArr addObject:@{ @"id" : [NSNumber numberWithInteger:versionsId],
                @"name" : nameStr }];
        }
    }
    [localDicDB close];
...
+ (NSString*)getLocalDicNameWithType:(LocalDicType)type
{
    ...
    else if (type == LocalDicTypeJobTypeLogo) {
        return @"v1jobtypelogo";
    }
...
+ (NSString*)getNameWithType:(LocalDicType)type VersionId:(NSInteger)versionId
{
    ...
    if ([localDicDB open]) {
        ...
        NSString* checkSql = [NSString stringWithFormat:@"SELECT * FROM DicVersions WHERE dicName = '%@' AND versionsId = %ld", dicName, (long)versionId];
        FMResultSet* result = [localDicDB executeQuery:checkSql];
