...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        if (customView) {
            ...
            if ([self dzn_buttonBordeWidth]!=0) {
                view.button.layer.borderWidth = [self dzn_buttonBordeWidth];
                view.button.layer.borderColor = [self dzn_buttonBorderColor].CGColor;
            }
            view.button.backgroundColor = [self dzn_buttonBackColor];
        }
...
- (void)updateConstraints
{
    ...
    if ([self canShowButton]) {
        ...
        [self.contentView addConstraint:[NSLayoutConstraint constraintWithItem:_button attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1.0 constant:_buttonSize.width]];
        [self.contentView addConstraint:[NSLayoutConstraint constraintWithItem:_button attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1.0 constant:_buttonSize.height]];
